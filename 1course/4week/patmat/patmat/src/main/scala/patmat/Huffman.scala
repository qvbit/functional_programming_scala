...
trait Huffman extends HuffmanInterface:
  ...
  def until(done: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] =
    // This is an O(N^2) algorithms. Can do this in O(NlogN) if we use a Priority Queue.
  ...
  def createCodeTree(chars: List[Char]): CodeTree = 
    ...
    val orderedLeaves = makeOrderedLeafList(charCount)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = 
    def encodeChar(subTree: CodeTree, char: Char): List[Bit] = subTree match
      case Leaf(char, weight) => Nil  // Leaf node marks the end of bit string which is now fully computed. Just mark end with Nil
      case Fork(left, right, chars, weight) =>
        if contains(left, char) then 0 :: encodeChar(left, char)
        else 1 :: encodeChar(right, char)

    def contains(subTree: CodeTree, char: Char): Boolean = subTree match
      case l: Leaf => l.char == char
      case f: Fork => f.chars.contains(char)

    if text.isEmpty then Nil
    else encodeChar(tree, text.head) ::: encode(tree)(text.tail)  // We need triple colon (concat) since left is not a single element, its a list
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] =
    ...
    val coder = codeBits(table)
    
    if text.isEmpty then Nil
    else coder(text.head) ::: quickEncode(tree)(text.tail)  // See above for why triple colon needed
